...
##### define the setting transmission from Pi to Arduino
...
def transmitMsg(Num):
#    s1.flushOutput()
#    mes =bytes(Num,'utf-8')    #### convert it to bytes and send
#    print(mes)
    s1.write(Num)
...
#    s1.flushInput()
    ...
    if s1.inWaiting() > 0:
        ...
        print('the message receive:',int.from_bytes(inputValue, big)) 
    else:
        print("Empty message:")
...
def ImagePreProcess(im_orig):
	...
	transmitMsg((str(ans)).encode('ascii'))
#	transmitMsg(ans.to_bytes(length = 1, byteorder = 'big'))
#   transmitMsg(bytes(ans))
	time.sleep(0.001)
#	receiveMsg()
	...
	while True:
		...
		try:
			...
			if key == ord("q"):
				...
				vs.stop()
#			elif key == ord("t"):
			   #    cv2.imwrite("dataset/User." + str(2) + '.' + str(count) + ".jpg", gray[y:y+h,x:x+w])
#				cv2.imwrite("num.jpg", frame)  
#				im_orig = cv2.imread("num.jpg")
#				ImagePreProcess(im_orig)
#			else:
